...
const PurnimaDetails = ({ slug, purnimaDetailData }: PurnimaDetailsProps) => {
  ...
  const recommendedData = useMemo(() => {
    ...
    const purnimas = purnimaDetailData?.PurnimaBlock?.find((item: { __component: string }) => item?.__component === 'shared.related-purnima')?.purnimas?.map((purnima: any) => ({ ... purnima, type: 'Purnima', id: `purnima-${purnima.id}`}));
    const pradosh = purnimaDetailData?.PurnimaBlock?.find((item: { __component: string }) => item?.__component === 'shared.related-pradosh')?.pradoshes?.map((pradosh: any) => ({ ... pradosh, type: 'Pradosh', id: `pradosh-${pradosh.id}`}));
    const ekadashis = purnimaDetailData?.PurnimaBlock?.find((item: { __component: string }) => item?.__component === 'shared.related-ekadashi')?.ekadashis?.map((ekadashi: any) => ({ ... ekadashi, type: 'Ekadashi', id: `ekadashi-${ekadashi.id}`}));
    const amavasyas = purnimaDetailData?.PurnimaBlock?.find((item: { __component: string }) => item?.__component === 'shared.related-amavasya')?.amavasyas?.map((amavasya: any) => ({ ... amavasya, type: 'Amavasya', id: `amavasya-${amavasya.id}`}));
  ...
  return (
    <div style={{ paddingBottom: 0 }}>
      ...
      {purnimaDetailData?.NextPurnimaLink?.purnimas?.[0]?.Title && (<div className="eka-next">
