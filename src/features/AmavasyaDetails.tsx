...
import { fetchAmavasyaDetails } from '../store/detailSlice';
import { AppDispatch, RootState } from '../store/store';
import { formatDate, formatDateTime, getDayFromDate } from '../common/functions';

const AmavasyaDetails = ({ slug }: { slug: string }) => {
  const dispatch = useDispatch<AppDispatch>();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<string>('eka-significance');

  const { amavasyaDetailData } = useSelector((state: RootState) => state.detail);
  useEffect(() => {
      dispatch(fetchAmavasyaDetails(slug))
  }, [slug]);
